defmodule Membrane.NodeProxy.Tee do
  @moduledoc """
  Element which can send buffers to other nodes and/or to elements on local nodes.
  """
  use Membrane.Filter
  ...
  def_output_pad :copy, availability: :on_request, caps: :any, mode: :push
  ...
  def handle_pad_added(Pad.ref(:copy, _ref), _ctx, state), do: {:ok, state}
  ...
  def handle_pad_removed(Pad.ref(:copy, _ref), _ctx, state), do: {:ok, state}
  ...
  @impl true
  def handle_process_list(:input, buffers, _cxt, %{enabled?: false} = state),
    do: {{:ok, forward: buffers, demand: {:input, length(buffers)}}, state}
  ...
  def handle_process_list(:input, buffers, ctx, state) do
    ...
    {{:ok, forward: buffers, demand: {:input, length(buffers)}}, state}
  end

  @impl true
  def handle_process(:input, buffer, _cxt, %{enabled?: false} = state),
    do: {{:ok, forward: buffer, demand: :input}, state}
  ...
  def handle_process(:input, %Membrane.Buffer{} = buffer, _ctx, state) do
    ...
    {{:ok, forward: buffer, demand: :input}, state}
  end
